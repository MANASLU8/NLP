    ...
    with open(document_path, "r") as f:
        ...
        while line:
            if line:
                token = line.split("\t")[0]
                if token != '\n':
                    tokens.append((line.split("\t")[2], line.split("\t")[3][:-1]))
            line = f.readline()
    f.close()
    return tokens
    ...
    for subdir, dirs, files in os.walk(annotation_documents_dir_path):
        for file in files:
            current_file_path = str(
                glob.glob(annotation_documents_dir_path + "/**/" + str(file_counter) + ".tsv", recursive=True)[0])
            ...
            for token_info in doc_tokens_list:
                if (token_info[0].lower() not in eng_stopwords) and (token_info[1] != "punctuation") \
